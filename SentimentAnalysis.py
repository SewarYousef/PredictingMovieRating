...
df['tokenized_text'] = df['tokenized_text'].apply(clean)
print(df.head())
...
bow_transformer = CountVectorizer(analyzer=clean).fit(df['tokenized_text'])
tweet_bow = bow_transformer.transform(df['tokenized_text'])

tfidf_transformer = TfidfTransformer().fit(tweet_bow)
tweet_tfidf = tfidf_transformer.transform(tweet_bow)

# Using naive bayes classifier
classifier_model = MultinomialNB().fit(tweet_tfidf, df['tokenized_text'])
all_predictions = classifier_model.predict(tweet_tfidf)
print(classification_report(df['tokenized_text'], all_predictions))
print(confusion_matrix(df['tokenized_text'], all_predictions))
print(accuracy_score(df['tokenized_text'], all_predictions))
...
tf_vector = tweet_tfidf(np.array(df.iloc[:, 1]).ravel())
X = tf_vector.transform(np.array(df.iloc[:, 1]).ravel())
y = np.array(df.iloc[:, 0]).ravel()

# Model Validation
tweet_train, tweet_test, sentiment_train, sentiment_test = train_test_split(X, y, test_size=0.2, random_state=30)
...
classifier_model = LogisticRegression().fit(tweet_tfidf, df['tokenized_text'])
all_predictions = classifier_model.predict(tweet_tfidf)
print(classification_report(df['tokenized_text'], all_predictions))
print(confusion_matrix(df['tokenized_text'], all_predictions))
print(accuracy_score(df['tokenized_text'], all_predictions))
